...
const fetchManifest = async ({ cache = true, root = '', recurse = false }:
	...
	manifest?.contents && await Promise.all(Object.keys(manifest?.contents)?.map(async key => {
		...
		let	contents = { name: key, link: (externalLink) ? key : `${root}/${key}` };
		if (entry?.type === 'folder' && recurse) {
			// fetch manifest for folder
			contents = { type: 'folder', ...contents, ...await fetchManifest({ cache, root: `${root}/${key}`, recurse })};
			...
			const body = await fetch(`${root}/${key}`,
					...
					.then(res => res.json())
					.catch(err => { return { error: 'MISSING_MANIFEST' } });
			contents = { ...contents, ...entry, type: entry.type || guessContentType(key) || 'application/octet-stream', json: { ...body } }
		} else {
			// else just add this entry
			contents = { ...contents, ...entry, type: entry.type || guessContentType(key) || 'application/octet-stream' };
		}
		new_manifest.contents.push(contents);
	}));
